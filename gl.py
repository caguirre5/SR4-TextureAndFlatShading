...
class Renderer(object):
    ...
    def glTriangle2(self, A, B, C, texCoords=(), normals=(), clr=None):
        ...
        triangleNormal = glMath.Cross(
            glMath.Substract(B, A), glMath.Substract(C, A))
        # normalizar
    ...
    def glGradientBG(self, clr=None):
        for y in range(self.height):
            for x in range(self.width):
                #Cuadrante -x-y
                if x < (self.width/2):
    ...
    def glCreateRotationMatrix(self, pitch=0, yaw=0, roll=0):
        ...
        temp = glMath.MultiplyMatrix(pitchMat, yawMat)
        matrix = glMath.MultiplyMatrix(temp, rollMat)
        return matrix
    ...
    def glCreateObjectMatrix(self, translate=V3(0, 0, 0), rotate=V3(0, 0, 0), scale=V3(1, 1, 1)):
        ...
        temp = glMath.MultiplyMatrix(translation, rotation)
        matrix = glMath.MultiplyMatrix(temp, scaleMat)
        return matrix
